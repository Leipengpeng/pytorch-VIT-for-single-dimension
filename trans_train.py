...
import torch
from self_supervised.ResNet.ResNet import ResNet
from self_supervised.Transformer.trans import VIT

def get_hyperparameters():
    parser = argparse.ArgumentParser(description="Training script")
    parser.add_argument('--root_addr', type=str, default="../../dataset/FS-SEI_4800")
    parser.add_argument('--label_num', type=int, default=10, choices=[10, 30, 50, 90])
...
def train(train_loader, model, criterion, optimizer, device, epochs, theta):
...
if __name__ == "__main__":
    ...
    train_loader = get_dataloader(root_addr=args.root_addr, label_num=args.label_num, batch_size=args.batch_size)
    theta = args.theta
    model = VIT()
    ...
    train(train_loader, model, criterion, optimizer, device, epochs, theta)
